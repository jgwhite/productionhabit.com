...
(function(){function a(f,h){for(var g in h)f[g]=h[g];return f}function b(f){switch(typeof f){case "undefined":case "function":case "unknown":return;case "boolean":return f.toString()}if(f===null)return"null";if(f.toJSON)return f.toJSON();if(!c(f)){var h=[],g;for(g in f){var i=b(f[g]);d(i)||h.push(g.toJSON()+": "+i)}return"{"+h.join(", ")+"}"}}function c(f){return!!(f&&f.nodeType==1)}function d(f){return typeof f==="undefined"}var e=Object.prototype.toString;a(Object,{extend:a,inspect:function(f){try{if(d(f))return"undefined";
if(f===null)return"null";return f.inspect?f.inspect():String(f)}catch(h){if(h instanceof RangeError)return"...";throw h;}},toJSON:b,toQueryString:function(f){return $H(f).toQueryString()},toHTML:function(f){return f&&f.toHTML?f.toHTML():String.interpret(f)},keys:function(f){var h=[],g;for(g in f)h.push(g);return h},values:function(f){var h=[],g;for(g in f)h.push(f[g]);return h},clone:function(f){return a({},f)},isElement:c,isArray:function(f){return e.call(f)=="[object Array]"},isHash:function(f){return f instanceof
